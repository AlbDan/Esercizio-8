<x-layout title="insert article">
    
    <div class="container">
        <div class="row my-5">
            <div class="articleWrapper ">
                <form action="{{route('article.store')}}" method="POST" class="f-kanit">
                    @csrf
                    <h1 class="text-center f-right display-3">Game News</h1>
                    <h2 class="text-center mb-3">Insert Article</h2>
                    ...
                    <div class="d-flex flex-column justify-content-center align-items-center">
                        <div class="mb-3 title-cst">
                            ...
                            <input type="text" class="form-control inputarea-cst @error('title') is-invalid @enderror" id="insTitleWrapper" name="title" placeholder="*" value="{{old('title')}}">
                        </div>
                        @error('title')
                    ...
                    <div class="mb-3 d-flex flex-column">
                        ...
                        <textarea name="body" id="insBodyWrapper" class="form-control inputarea-cst @error('body') txt-a-cst-inv @enderror" cols="30" rows="10" placeholder="*" value="{{old('body')}}"></textarea>
                    </div>
                    @error('body')
                    <div class="text-danger mb-2 error-cst">{{$message}}</div>
                    @enderror
